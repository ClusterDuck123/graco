...
double* get_weights();
double** signature_similarity4(int GDV[][15], int m);
...
// Create empty distance matrix
  ...
  double WEIGHT_SUM = std::accumulate(WEIGHTS, WEIGHTS+15, 0);

  for (auto u = 0; u < m; u++) {
    for (auto v = u+1; v < m; v++) {
      for (auto i = 0; i < 15; i++) {
        D[u][v] += WEIGHTS[i] * std::abs(log_GDV[u][i] - log_GDV[v][i]) /
